...
def get_inspection_page(**kwargs):
    """Retrieve the search parameters data from web."""
...
def extract_data_listings(html):
    """Return the separated listings."""
    id_finder = re.compile(r'PR[\d]+~')
    return html.find_all('div', id=id_finder)


def has_two_tds(elem):
    """Return the correct table data."""
    is_tr = elem.name == 'tr'
...
def extract_restaurant_metadata(elem):
    """Return restraunt meta data from site."""
...
def is_inspection_row(elem):
    """Return row element with insepction data."""
    is_tr = elem.name == 'tr'
    if not is_tr:
        return False
    td_children = elem.find_all('td', recursive=False)
...
def extract_score_data(elem):
    """Extract relavent score information into a dictionary."""
...
def generate_results(test=False, count=15):
    """Gernerate 25 results for mapping."""
    kwargs = {
...
def get_geojson(result):
    """Return geojson data for mapping."""
